...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    if num_rolls == 0:
        """ Free bacon rule. """
        score_digits = [int(x) for x in str(opponent_score)]
        return max(score_digits) + 1
    else:
...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    while max(scores) < goal:
        my_strategy = strategies[who]
        my_score = scores[who]
        my_opponent_score = scores[other(who)]

        num_rolls = my_strategy(my_score, my_opponent_score)
        dice = select_dice(my_score, my_opponent_score)
        scores[who] = my_score + take_turn(num_rolls, my_opponent_score, dice)

        if max(scores) / 2 in scores:
            scores = [scores[1], scores[0]]
        who = other(who)

    return scores[0], scores[1]  # You may wish to change this line.
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    """
    "*** YOUR CODE HERE ***"
    num_rolls, max_rolls = 1, 10
    averaged_dice = make_averaged(roll_dice, 10000)
    results = []

    while num_rolls <= max_rolls:
        result = averaged_dice(num_rolls, dice)
        results.append(result)
        print(num_rolls, "dice scores", result, "on average")
        num_rolls = num_rolls + 1

    max_result = max(results)
    max_result_dice = results.index(max_result) + 1

    return max_result_dice
...
def bacon_strategy(score, opponent_score):
    ...
    "*** YOUR CODE HERE ***"
    score_digits = [int(x) for x in str(opponent_score)]
    bacon_score = max(score_digits) + 1
...
def swap_strategy(score, opponent_score):
    ...
    "*** YOUR CODE HERE ***"
    score_digits = [int(x) for x in str(opponent_score)]
    swap_score = score + max(score_digits) + 1

    if swap_score * 2 == opponent_score:
        return 0
    elif swap_score / 2 == opponent_score:
        return BASELINE_NUM_ROLLS
    else:
...
def final_strategy(score, opponent_score):
    ...
    If the bacon score leaves opponent with 4-sided dice, use that.
    """
    "*** YOUR CODE HERE ***"
    score_digits = [int(x) for x in str(opponent_score)]
    bacon_score = max(score_digits) + 1
    ...
    if dice == four_sided and bacon_score >= 5:
        return 0
    elif dice == six_sided and bacon_score >= 9:
        return 0
    elif swap_score * 2 == opponent_score:
        return 0
    elif swap_score / 2 == opponent_score:
        ...
        if score - opponent_score <= -cutoff:
            return BASELINE_NUM_ROLLS + 1
        else:
            return BASELINE_NUM_ROLLS - 1
    else:
        if score - opponent_score <= -cutoff:
            return BASELINE_NUM_ROLLS + 4
        else:
