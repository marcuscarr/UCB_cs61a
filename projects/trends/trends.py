...
def make_tweet_fn(text, time, lat, lon):
    ...
    def tweet(var):
        if var == 'text':
            ...
            print("Incorrect call to tweet with ", var)
...
def analyze_tweet_sentiment(tweet):
    ...
    for word in words:
        ...
        if has_sentiment(word_sentiment):
            word_value = sentiment_value(word_sentiment)
            total, count = total + word_value, count + 1
    if count == 0:
...
def find_state_center(polygons):
    ...
    return make_position(wtsum_lat / sum_area, wtsum_lon / sum_area)
