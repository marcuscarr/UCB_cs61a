    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant():
            ...
            if self.ant is None:
                ...
                raise AssertionError('Ants {2} in {0} and neither {2} nor \
{1} can contain the other.'.format(self, insect, self.ant))
                return
        else:
    ...
    def remove_insect(self, insect):
        ...
        if not insect.is_ant():
            ...
            if self.ant.container:
                assert self.ant == insect or self.ant.ant == insect, '{0} not in {1}'.format(insect, self)
            else:
                assert self.ant == insect, '{0} not in {1}'.format(insect, self)
            "*** YOUR CODE HERE ***"
            if type(insect) == QueenAnt and not insect.impostor_queen:
                return
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        # Jump ahead to first Place in range
        for _ in range(self.min_range):
            target_place = target_place.entrance
            target_range += 1

        while target_place is not hive and target_place is not None:
...
class QueenPlace:
    ...
    def __init__(self, colony_queen, ant_queen):
        "*** YOUR CODE HERE ***"
        self.place_list = [colony_queen, ant_queen]
...
class QueenAnt(ScubaThrower):
    ...
    def action(self, colony):
        ...
        tunnel_places = [self.place]
        this_entrance = self.place.entrance
        this_exit = self.place.exit

        while this_entrance is not None:
            tunnel_places.append(this_entrance)
            this_entrance = this_entrance.entrance
        while this_exit is not None:
            tunnel_places.append(this_exit)
            this_exit = this_exit.exit

        for place in tunnel_places:
            if place.ant is not None and place.ant not in QueenAnt.doubled_ants:
                if type(place.ant) == QueenAnt:
                    continue
                place.ant.damage *= 2
                QueenAnt.doubled_ants.append(place.ant)

                # Check for Ant within a BodyguardAnt
                if place.ant.container and place.ant.ant is not None:
                    place.ant.ant.damage *= 2
                    QueenAnt.doubled_ants.append(place.ant.ant)
